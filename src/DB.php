...
class DB {
  ...
  public static $connect_options = array(
  ...
  protected static function connectParam() {
    // On se connecte
    if (self::$autocommit) {
      $bdd = new PDO('mysql:host=' . DB::$host . ';dbname=' . DB::$dbName, DB::$user, DB::$password, array(
...
class MeekroDB {
  ...
  public function sync_config() {
    $db_class_vars = get_class_vars('DB'); // the DB::$$var syntax only works in
                                           // 5.3+
  ...
  public function nonSQLError($message) {
    ...
    call_user_func(__NAMESPACE__ . ' ' . $error_handler, array(
      'type' => 'nonsql',
      'error' => $message
    ));
  }
  ...
  public function insertOrReplace($which, $table, $datas, $options = array()) {
    ...
    if (isset($options['update']) && is_array($options['update']) && $options['update'] && strtolower($which) == 'insert') {
      if (array_values($options['update']) !== $options['update']) {
        return $this->query(str_replace('%', $this->param_char, "INSERT INTO %b %lb VALUES $var ON DUPLICATE KEY UPDATE %hc"), $table, $keys, $values, $options['update']);
      }
      else {
    ...
    return $this->query(str_replace('%', $this->param_char, "%l INTO %b %lb VALUES $var"), $which, $table, $keys, $values);
  }
  ...
  public function insertUpdate() {
    ...
    if (!isset($args[0])) { // update will have all the data of the insert
      if (isset($data[0]) && is_array($data[0])) { // multiple insert rows
                                                   // specified -- failing!
  ...
  protected function preparseQueryParams() {
    ...
    $types = array(
      ...
      $this->param_char . 'ss' // search string (like string, surrounded with
                               // %'s)
    );
    ...
    // for each MeekroDB variable, substitute it with array(type: i, value: 53)
    // or whatever
    $chunkyQuery = array(); // preparsed query
    $pos_adj = 0; // how much we've added or removed from the original sql
                  // string
    foreach ($posList as $pos => $type) {
      $type = substr($type, $param_char_length); // variable, without % in front
                                                 // of it
      $length_type = strlen($type) + $param_char_length; // length of variable
                                                         // w/o %
      
      $new_pos = $pos + $pos_adj; // position of start of variable
      $new_pos_back = $new_pos + $length_type; // position of end of variable
      $arg_number_length = 0; // length of any named or numbered parameter
                              // addition
      ...
      if ($arg_number_length = strspn($sql, '0123456789', $new_pos_back)) {
        ...
        if (substr($sql, $new_pos_back, $named_seperator_length) == $this->named_param_seperator) {
      ...
      if (is_object($value)) {
        if ($value instanceof MeekroDBEval)
          ...
          if ($value instanceof DateTime)
            ...
            return $this->escape($value); // use __toString() value for objects,
                                          // when possible
      }
  ...
  protected function queryHelper() {
    ...
    // mysqli_result->num_rows won't initially show correct results for
    // unbuffered data
...
class WhereClause {
  ...
  // we now return full WhereClause object here and evaluate it in
  // preparseQueryParams
  function text() {
    return $this;
  }
...
class DBHelper {
  ...
  /*
   * verticalSlice
   * 1. For an array of assoc rays, return an array of values for a particular
   * key
   * 2. if $keyfield is given, same as above but use that hash key as the key in
   * new array
   */
  ...
  /*
   * reIndex
   * For an array of assoc rays, return a new array of assoc rays using a
   * certain field for keys
   */
